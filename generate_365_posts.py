...
"""
GENERATOR 365 POSTÓW - TRUE RESULTS ONLINE
==========================================
Generuje 365 artykułów z datami cofnięte od dziś (23.09.2025) wstecz
...
def load_config_from_excel(path: Path) -> dict:
    ...
    try:
        ...
        # Wczytywanie matrycy case studies
        ...
        print(f"✅ Wczytano {len(campaign_topics)} pakietów case study.")
        return config
...
def pick_topic_package_cycling(config: dict, post_index: int) -> dict:
    """Wybiera pakiet tematyczny cyklicznie (bez historii użycia)."""
    campaign_topics = config.get('campaign_topics', [])
    if not campaign_topics:
        print("❌ Brak dostępnych pakietów case study w konfiguracji.")
        return None
    
    # Cykliczny wybór bez duplikatów (modulo)
    chosen_package = campaign_topics[post_index % len(campaign_topics)]
    
    # Dodaj unikalny suffix dla każdego użycia
    cycle_number = (post_index // len(campaign_topics)) + 1
    if cycle_number > 1:
        chosen_package = chosen_package.copy()
        chosen_package['title'] = f"{chosen_package['title']} - Część {cycle_number}"
...
def main():
    """Główna funkcja generująca 365 postów."""
    print("=== GENERATOR 365 POSTÓW - TRUE RESULTS ONLINE ===")
    ...
    # Generowanie 365 postów
    for i in range(365):
        target_date = start_date - datetime.timedelta(days=i)
        
        print(f"\n📅 POST {i+1}/365 - Data: {target_date.strftime('%Y-%m-%d')}")
        
        # Wybór tematu (cykliczny)
        topic_package = pick_topic_package_cycling(config, i)
        ...
        if (i + 1) % 50 == 0:
            print(f"🔄 Postęp: {i+1}/365 postów...")
            time.sleep(2)
    
    print(f"\n🎯 WYGENEROWANO {len(posts_data)} POSTÓW")
    ...
    try:
        ...
        if repo.is_dirty(untracked_files=True):
            commit_message = f"Automatyczne wygenerowanie 365 artykułów - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"
    ...
    print("\n🎉 SUKCES!")
    print(f"✅ Wygenerowano {len(posts_data)} artykułów")
